...
> **注意**: このセクションはローカル補助ガイドラインである。Go API プロジェクトの正本は `api-test-strategy.md` の「## 5. 契約ドキュメント」を参照。

AAA パターンと契約プログラミングの対応関係:

| AAA | 契約 | 内容 |
|-----|------|------|
| Arrange | 事前条件 | テスト実行前に満たすべき状態（DB データ、モック設定） |
| Act | 操作 | テスト対象のメソッド呼び出しや HTTP リクエスト |
| Assert | 事後条件 | テスト実行後に成り立つべき状態（レスポンス、戻り値） |

### 原則

- Integration test では事前条件を fixture で満たすことを原則とする
- ただし一回限りの状態遷移や例外状態は直接 SQL / 最小 setup 可
- 事前条件には結果状態を書く（helper 名ではなく）
- 事前条件に操作を書かない（HTTP リクエスト送信やメソッド呼び出しは操作）
